...
import { cloneAndScan } from '../lib/git';
...
fs.appendFileSync('api-debug.log', `[${new Date().toISOString()}] API Queue initialized with Redis: ${redisHost}:6379\n`);
...
router.post('/:id/detect-roots', authenticate, async (req: AuthRequest, res: Response) => {
    try {
        ...
        const roots = await cloneAndScan(project.repoUrl, project.branch);
        res.json({ roots });
    } catch (error) {
